...
protocol FeedCellNavigationDelegate: class {
...
class FeedTableViewCell: UITableViewCell {
    ...
    var post: Result?
    weak var delegate: FeedCellNavigationDelegate?
    ...
    func configure(post: Result, tag : Int){
        self.tag = tag
        self.post = post
        nameBtn.setTitle("@" + post.creatorUsername!, for: .normal)
        nameBtn.titleLabel?.font = UIFont.systemFont(ofSize: 18, weight: .semibold)
        musicLbl.text = "Hi hello Welcome Hi hello Welcome Hi hello Welcome Hi hello Welcome Hi hello Welcome Hi hello Welcome Hi hello Welcome"//post.descriptionField // Long enough to enable scrolling
        captionLbl.text = post.musicText
        likeCountLbl.text = post.numLikes!.shorten()
        //commentCountLbl.text = post.comments?.count.shorten()
        shareCountLbl.text = post.shares!.shorten()
        
        playerView.configure(url: URL(string: post.file!), feed: post)
    }
    ...
    @objc func navigateToProfilePage(){
        ...
        delegate?.navigateToProfilePage(uid: post.id, name: post.creatorUsername!)
